...
*When it is true, we get an error that pops up that isnt specific. When it's false, it allows for us to stop our program and let the user knw the error with a personalized message.*
...
```{r}
function_weights <- function(x) {
  x / sum(x, na.rm = TRUE)
}

function_weights(temp)
function_weights()
```
...
mpg |>
  group_by(class) |>
  summarize(across(cty:hwy, function_SE))
...
flights |>
  group_by(carrier) |>
  summarize(across(c(), function_SE))
```
...
```{r}
function_SE <- function(x, removeNAs = FALSE) {
  n = sum(!is.na(x))
  sqrt(var(x, na.rm = removeNAs)/n)
}


temp <- c(4, 6, 8, 9)
function_SE(temp, removeNAs = TRUE)
