...
groups and order connect dots on map
...
vacc_mar13 |>
  right_join(us_states, by = c("State" = "region")) |> # joins us_states  to all variables in vacc_mar13
  rename(region = State) |>
  ggplot(mapping = aes(x = long, y = lat,
                          group = group)) + 
...
The states we were missing on map are actually named different things and are included in dataset. helps check where the mismatch is.
...
leaflet(states) |>
  ...
  addPolygons(
    ...
    weight = 2,
    opacity = 1,
    color = "white", #state border
    dashArray = "3",
    fillOpacity = 0.7,
    highlightOptions = highlightOptions(
      weight = 5,
      color = "#666", # stateoutline darkness
      dashArray = "",
      fillOpacity = 0.7,
      bringToFront = TRUE),
    label = labels,
...
polisci_polygon <- states_polygon |>
  ...
  filter(!(region %in% c("alska", "hawaii", "puertorico")))
...
polisci_polygon_sf <- polisci_polygon |>
  left_join(states_sf, by = c("region" = "name")) |>
  filter(!(region %in% c("alska", "hawaii", "puertorico"))) 
...
cat_death_data <- cat_death_data |>
  mutate(labels = str_c(name, ": ", deaths, " Number of deaths recorded"))

labels <- lapply(cat_death_data$labels, HTML)

leaflet(cat_death_data) |>
  setView(-96, 37.8, 4) |>
  addTiles() |>
  addPolygons(
    data = cat_death_data$geometry,
